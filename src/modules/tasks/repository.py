...
# type: ignore
async def assign_task_to_user_repo(task: Task, student_id: UUID, session: DbSession): # type: ignore
    task.client_id = student_id
    await session.commit()
...
# type: ignore
async def unassign_task_from_user_repo(task: Task, session: DbSession): # type: ignore
    task.client_id = None
    await session.commit()
    return task
...
# type: ignore
async def assign_task_to_group_repo(task: Task, group_id: UUID, session: DbSession): # type: ignore
...
async def mark_task_as_complete_repo(task: Task, session: DbSession): # type: ignore
...
async def read_tasks_by_student_sort_start_time(student_id: UUID, session: DbSession): # type: ignore
    statement = select(Task).where(Task.client_id == student_id).order_by(Task.start_date)
    result = await session.execute(statement)
    return result.scalars().all()

async def read_tasks_by_student_sort_end_time(student_id: UUID, session: DbSession): # type: ignore
    statement = select(Task).where(Task.client_id == student_id).order_by(Task.due_date)
    result = await session.execute(statement)
    return result.scalars().all()

async def read_tasks_by_student_sort_status(student_id: UUID, session: DbSession): # type: ignore
    statement = select(Task).where(Task.client_id == student_id).order_by(Task.status)
    result = await session.execute(statement)
    return result.scalars().all()



async def read_tasks_by_user(client: Client, session: DbSession):  # type: ignore
    if client is None:
        return []
    return await read_tasks_by_student(client.id, session)
